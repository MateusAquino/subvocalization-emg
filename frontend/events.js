...
addWord.onclick = () => {addToList("[name=words]", "#word")}
deleteWord.onclick = () => {deleteFromList("[name=words]")}
addFallback.onclick = () => {addToList("[name=fallbacks]", "#fallback")}
deleteFallback.onclick = () => {deleteFromList("[name=fallbacks]")}
deleteRecording.onclick = () => {deleteFromList("[name=recordings]")}
...
networkTab.onclick = () => {
  const recordings = Array.from(document.querySelector("[name=recordings]").options).map(option => option.value);
  const included = Array.from(includedSelect.options);
  const excluded = Array.from(excludedSelect.options);
  const existingOptions = included.map(option => option.value).concat(excluded.map(option => option.value));
  const toAdd = recordings.filter(el => !existingOptions.includes(el));
  const toRemoveIncluded = included.filter(option => !recordings.includes(option.value));
  const toRemoveExcluded = excluded.filter(option => !recordings.includes(option.value));
...
deleteNetwork.onclick = () => {deleteFromList("[name=networks]")}
...
function deleteFromList(selector) {
  ...
  Array.from(selected).map(option => select.removeChild(option));
}

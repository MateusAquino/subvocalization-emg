...
def record_stream(saveasrec, wps, period, include_silence, include_fallback, words):
    if not saveasrec:
      saveasrec = ''.join(rd.choice(string.ascii_uppercase) for i in range(10))
    last_word = None
    loop_count = 0
    ...
    eel.train_step("$PREPARE", -1, total_loops)
    eel.sleep(3)
    get_board_data()
    ...
    data = get_board_data()
    eel.train_step("$END", loop_count, total_loops)
    transposed_data = np.transpose(limit_channels(data))
    word_history = np.transpose(word_history)

    eel.log("Saving to file...")
    df = pd.DataFrame(transposed_data, columns=['Channel_{}'.format(x) for x in range(1, transposed_data.shape[1]+1)])
